...
describe('session security', () => {
  ...
  it('preserves the absolute expiry reason when the capped refresh cookie is already gone', async () => {
    sessionStore.accept(
      metadata({ absoluteExpiresAt: new Date(Date.now() - 1_000).toISOString() }),
      true,
      false,
    )
  ...
  it('retains the original absolute deadline when refreshing', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(response({ ...session, accessToken: 'new', expiresIn: 600 })),
    )
  ...
  it('reports explicit activity without sending client timestamps', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      expect.stringMatching(/\/session\/activity$/),
  ...
  it('ignores older server snapshots delivered after a newer activity response', () => {
    const newer = metadata({
      serverTime: '2030-01-01T00:10:00Z',
      idleExpiresAt: '2030-01-01T00:40:00Z',
    })
    sessionStore.accept(newer)
    sessionStore.accept(
      metadata({ serverTime: '2030-01-01T00:09:00Z', idleExpiresAt: '2030-01-01T00:39:00Z' }),
    )
