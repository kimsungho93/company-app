...
import { clearPendingLogout, getPendingLogout, errorCode, isSessionEndCode, sessionEndReasonFor, sessionStore, withAuthLock } from './sessionStore'
...
const authChanged = () => ({ error: { status: 'CUSTOM_ERROR' as const, error: '인증 상태가 변경되었습니다.' } })
    ...
    let result = await (url === '/auth/logout' || url === '/auth/login'
      ? withAuthLock(async () => {
        if (generation !== tokenStore.generation() ||
            (url === '/auth/logout' && pendingLogout !== getPendingLogout())) return authChanged()
        return await raw(args, api, extraOptions)
      }).catch((cause: unknown) => ({
        error: { status: 'CUSTOM_ERROR' as const, error: cause instanceof Error ? cause.message : '로그인 동기화를 시작할 수 없습니다.' },
      }))
