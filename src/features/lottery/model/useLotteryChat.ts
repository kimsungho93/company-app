...
const combine = (previous: ChatMessage[], incoming: ChatMessage[], oldestSeq: number) => {
  ...
  incoming.forEach((message) => {
    ...
    const readers = new Map(
      [...(existing?.readers ?? []), ...message.readers].map((reader) => [reader.userId, reader]),
    )
    messages.set(message.seq, { ...message, readers: [...readers.values()] })
  })
  return [...messages.values()]
    .filter((message) => message.seq >= oldestSeq)
    .sort((a, b) => a.seq - b.seq)
    .slice(-1000)
}

export const useLotteryChat = ({
  transport,
  currentUser,
  open,
}: {
  ...
  const requests = useRef(
    new Map<
      string,
      { older: boolean; timer: ReturnType<typeof setTimeout>; typingRevision: number }
    >(),
  )
  ...
  const requestHistory = useCallback(
    (older = false) => {
      ...
      if (
        !transport.isConnected() ||
        (older && (!current.hasMore || current.loading || current.loadingOlder))
      )
        return
      ...
      const timer = setTimeout(() => {
        ...
        update((previous) => ({
          ...
          loading: false,
          loadingOlder: false,
          error: '대화를 불러오지 못했어요. 다시 시도해 주세요.',
        }))
      }, 10_000)
  ...
  const setTyping = useCallback(
    (typing: boolean) => {
      ...
      if (!typingState.current.active || now - typingState.current.lastSent >= 2000) {
        if (transport.publish('typing', { typing: true }))
  ...
  const acceptTyping = useCallback(
    (people: ChatPerson[]) => {
      update((previous) => ({
        ...previous,
        typing: people.filter((person) => person.userId !== userId),
      }))
    },
    [update, userId],
  )
    ...
    const receive = (event: LotteryChatEvent) => {
      ...
      if (event.type === 'PAGE') {
        ...
        const alreadyRead = new Set(
          event.messages
            .filter((message) => message.readers.some((reader) => reader.userId === userId))
        ...
        resolved.forEach((id) => {
          clearTimeout(sends.current.get(id))
        ...
        update((previous) => ({
          ...
          confirmed: combine(
            request.older
              ? previous.confirmed
              : previous.confirmed.filter(
                  (message) =>
                    message.seq > event.latestSeq ||
                    event.messages.some((item) => item.seq === message.seq),
                ),
            event.messages,
            oldestSeq,
          ),
        ...
        update((previous) => ({
          ...
          confirmed: combine(
            previous.confirmed,
            matches(message, previous) ? [message] : [],
            oldestSeq,
          ),
          oldestSeq,
          pending:
            message.senderId === userId
              ? previous.pending.filter((item) => item.clientMessageId !== message.clientMessageId)
              : previous.pending,
          latestSeq: Math.max(previous.latestSeq, message.seq),
          unread:
            message.seq >= oldestSeq &&
            message.senderId !== userId &&
            !readSent.current.has(message.seq) &&
            !message.readers.some((reader) => reader.userId === userId)
              ? [...new Set([...previous.unread, message.seq])]
                  .filter((seq) => seq >= oldestSeq)
                  .slice(-1000)
              : previous.unread.filter((seq) => seq >= oldestSeq),
        }))
      } else if (event.type === 'READ') {
        const seqs = new Set(event.seqs)
        update((previous) => ({
          ...previous,
          confirmed: previous.confirmed.map((message) =>
            seqs.has(message.seq) &&
            !message.readers.some((reader) => reader.userId === event.reader.userId)
              ? { ...message, readers: [...message.readers, event.reader] }
              : message,
          ),
          unread:
            event.reader.userId === userId
              ? previous.unread.filter((seq) => !seqs.has(seq))
              : previous.unread,
        }))
          ...
          update((previous) => ({
            ...
            pending: previous.pending.map((message) =>
              message.clientMessageId === event.clientMessageId
                ? { ...message, delivery: 'failed', failure: event.message }
                : message,
            ),
          }))
        ...
        pending: previous.pending.map((message) =>
          message.delivery === 'sending'
            ? {
                ...message,
                delivery: 'failed',
                failure: '연결이 끊겼어요. 다시 연결한 뒤 재시도해 주세요.',
              }
            : message,
        ),
      }))
  ...
  const publishMessage = useCallback(
    (message: ChatDisplayMessage) => {
      ...
      const timer = setTimeout(() => {
        ...
        update((previous) => ({
          ...
          pending: previous.pending.map((item) =>
            item.clientMessageId === message.clientMessageId && item.delivery === 'sending'
              ? {
                  ...item,
                  delivery: 'failed',
                  failure: '전송을 확인하지 못했어요. 재시도해 주세요.',
                }
              : item,
          ),
        }))
      ...
      if (
        !transport.publish('send', { clientMessageId: message.clientMessageId, text: message.text })
      ) {
        clearTimeout(timer)
        sends.current.delete(message.clientMessageId)
        update((previous) => ({
          ...
          pending: previous.pending.map((item) =>
            item.clientMessageId === message.clientMessageId
              ? { ...item, delivery: 'failed', failure: '전송하지 못했어요. 다시 시도해 주세요.' }
              : item,
          ),
        }))
      }
  ...
  const send = useCallback(
    (value: string) => {
      ...
      if (stateRef.current.pending.length >= 20) {
        update((previous) => ({
          ...previous,
          error: '전송하지 못한 메시지를 먼저 재시도해 주세요.',
        }))
        return false
      }
  ...
  const retry = useCallback(
    (clientMessageId: string) => {
      const message = stateRef.current.pending.find(
        (item) => item.clientMessageId === clientMessageId && item.delivery === 'failed',
      )
      ...
      update((previous) => ({
        ...
        pending: previous.pending.map((item) =>
          item.clientMessageId === clientMessageId
            ? { ...item, delivery: 'sending', failure: undefined }
            : item,
        ),
      }))
      publishMessage(message)
  ...
  const markRead = useCallback(
    (seqs: number[]) => {
      if (!openRef.current || document.visibilityState !== 'visible' || !transport.isConnected())
        return
      ...
      for (let offset = 0; offset < unread.length; offset += 100) {
        ...
        if (transport.publish('read', { seqs: batch }))
          batch.forEach((seq) => readSent.current.add(seq))
      }
      update((previous) => ({
        ...previous,
        unread: previous.unread.filter((seq) => !readSent.current.has(seq)),
      }))
    },
