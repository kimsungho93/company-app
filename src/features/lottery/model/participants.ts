...
export const validateLotterySettings = ({
  participants,
  winnerCount,
}: LotterySettings): string | null => {
  if (participants.length < 1 || participants.length > 50)
    return '참가자는 1~50명까지 등록할 수 있습니다.'
  const names = participants.map((name) => name.trim())
  if (names.some((name) => name.length < 1 || name.length > 30))
    return '참가자 이름을 1~30자로 입력해 주세요.'
