...
const input: LotteryResultImageInput = {
  ...
  winners: [{ name: '육이슬', drawnAt: '2026-09-14T00:00:00Z' }],
  winnerCount: 2,
  finished: false,
}
...
describe('result image download', () => {
  ...
  it('downloads a PNG with a safe filename and releases the temporary link and URL', async () => {
    ...
    await act(async () => {
      await result.current.saveImage(input)
    })
    expect(createUrl).toHaveBeenCalledWith(blob)
    expect(downloads).toEqual([
      { filename: '점심추첨결과-당첨 결과.png', url: 'blob:lottery-result' },
    ])
  ...
  it('captures the clicked results once while new winners arrive and ignores duplicate clicks', async () => {
    ...
    createImage.mockImplementation(
      () =>
  ...
  it.each(['reset', 'leave'])(
    'does not download an old image after %s during rendering',
    async (action) => {
      let resolve!: (blob: Blob) => void
      createImage.mockImplementation(
        () =>
      ...
      const { result, rerender, unmount } = renderHook(
        ({ scope }) => useResultImageDownload(scope),
        { initialProps: { scope: 'room:1:false' } },
      )
      let pending!: Promise<void>
      act(() => {
        pending = result.current.saveImage(input)
      })
  ...
  it('does not create an image before any winner has been announced', async () => {
    const { result } = renderHook(() => useResultImageDownload('room:1'))
    await act(async () => {
      await result.current.saveImage({ ...input, winners: [] })
    })
