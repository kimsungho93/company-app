...
export const GATE_POSITION: BallPosition = [0, -0.6, 0.36]
...
export const swirlingPosition = (
  ...
  const radius =
    (GLOBE_RADIUS - ballRadius(count) - 0.06) * (0.76 + 0.2 * Math.sin(time * 1.5 + phase))
...
const insideDrum = (position: BallPosition, count: number): BallPosition => {
  ...
  return [
    position[0] * scale,
    (position[1] - GLOBE_CENTER_Y) * scale + GLOBE_CENTER_Y,
    position[2] * scale,
  ]
}

export const chutePosition = (progress: number): BallPosition =>
  cubicPosition(GATE_POSITION, CHUTE_ENTRY, [0, -1.29, 0.64], CHUTE_END, progress)

export const rollingPosition = (progress: number): BallPosition =>
  cubicPosition(CHUTE_END, [0, -1.59, 0.88], [0.61, -1.55, 1.02], TRAY_END, progress)

export const exitingPosition = (
  from: BallPosition,
  elapsedMs: number,
  tangent: BallPosition = from,
): BallPosition | null => {
...
export const drawnBallPosition = (
  index: number,
  count: number,
  drawId: number,
  drawnAt: number,
  ...
  const from =
    origin && start === origin.startedAt
      ? origin.position
      : swirlingPosition(index, count, drawId, drawnAt)
  const current = swirlingPosition(index, count, drawId, start)
  const previous = swirlingPosition(index, count, drawId, start - 16)
  const tangent = insideDrum(
    from.map((value, axis) => value + (current[axis] - previous[axis]) * 9) as BallPosition,
    count,
  )
  const elapsed =
    serverNow < captureEndsAt
