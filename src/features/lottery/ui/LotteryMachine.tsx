    ...
    <div className={styles.fallback}>
      <div className={styles.fallbackGlobe}>
        <div className={styles.fallbackBalls}>
          {participants.filter((name) => !drawn.has(name)).slice(0, 50).map((name) => (
            <span key={name} className={styles.fallbackBall} style={{ '--ball-color': ballColor(participants.indexOf(name)) } as CSSProperties}>
              <span>{name}</span>
            </span>
          ))}
        </div>
      </div>
      <div className={styles.fallbackBase}><span>{status === 'DRAWING' ? '추첨 중' : 'LUCKY DRAW'}</span></div>
...
export const LotteryMachine = (props: LotteryMachineProps) => {
  ...
  const exitEndsAt = props.status !== 'READY' && lastDrawnAt ? Date.parse(lastDrawnAt) + EXIT_DURATION_MS : 0
  ...
  const animationEndsAt = serverNow < exitEndsAt ? exitEndsAt : settling ? exitEndsAt + SETTLE_DURATION_MS : 0
  ...
  return (
    <div className={styles.machine} role="img" aria-label={`이름이 적힌 공 ${remaining}개가 남은 추첨기. ${presenting ? '추첨 중입니다.' : props.status === 'FINISHED' ? '추첨이 완료되었습니다.' : '추첨을 기다리고 있습니다.'}`}>
      <div className={styles.ground} aria-hidden="true" />
      <MachineBoundary fallback={fallback}>
        {contextLost ? fallback : (
          <Canvas camera={{ position: [1.3, 0.8, 7.8], fov: 37, near: 0.1, far: 30 }} dpr={[1, 1.5]} shadows="percentage"
            gl={{ alpha: true, antialias: true, powerPreference: 'default' }} fallback={fallback}
            aria-hidden="true" frameloop={!reducedMotion && (presenting || settling) ? 'always' : 'demand'}>
            <MachineScene {...props} mixing={presenting} reducedMotion={reducedMotion} onContextLost={onContextLost} />
          </Canvas>
        )}
      </MachineBoundary>
      <div className={styles.caption} aria-hidden="true">
        <span className={`${styles.indicator} ${presenting ? styles.active : ''}`} />
        {presenting ? '행운의 주인공을 뽑고 있어요' : props.status === 'FINISHED' ? '행운의 주인공이 모두 정해졌어요' : `${props.participants.length}명의 행운을 담았어요`}
      </div>
    </div>
  )
}
