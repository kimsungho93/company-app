...
export const LotterySettingsPanel = ({ room, isHost, busy, connected, onSave, onStart }: Props) => {
  ...
  const dirty =
    JSON.stringify(names) !== JSON.stringify(room.participants) ||
    Number(winnerCount) !== room.winnerCount
  const invalid = validateLotterySettings(draft)
  const countLabel =
    Number.isInteger(draft.winnerCount) && draft.winnerCount > 0
      ? draft.winnerCount
      : room.winnerCount
  ...
  const add = () => {
    ...
    if (!trimmed) {
      setError('추가할 이름을 입력해 주세요.')
      return
    }
    if (trimmed.length > 30) {
      setError('이름은 30자까지 입력할 수 있어요.')
      return
    }
    if (names.includes(trimmed)) {
      setError('이미 명단에 있는 이름이에요.')
      return
    }
    if (names.length >= 50) {
      setError('최대 50명까지 넣을 수 있어요.')
      return
    }
    ...
    <section className={styles.panel} aria-labelledby={headingId}>
      <details ref={detailsRef} className={styles.roster}>
        <summary id={headingId}>
          추첨 명단 <span>· {names.length}명</span>
        </summary>
        <div className={styles.rosterBody}>
          <ul className={styles.names} aria-label="추첨 대상 명단">
            {names.map((person, index) => (
              <li key={person}>
                <span
                  className={styles.colorDot}
                  style={{ backgroundColor: ballColor(index) }}
                  aria-hidden="true"
                />
            ...
            >
              ...
              <div>
                <input
                  ...
                  placeholder="이름 입력"
                  autoComplete="off"
                  disabled={locked || names.length >= 50}
                ...
                />
                <Button type="submit" variant="secondary" disabled={locked || names.length >= 50}>
                  추가
                </Button>
              </div>
            </form>
      ...
      {editable ? (
        <>
          <div className={styles.countField}>
            ...
            <div className={styles.stepper}>
              <button
                ...
                disabled={locked || !names.length || draft.winnerCount <= 1}
              ...
              <input
                ...
                step={1}
                value={winnerCount}
                disabled={locked || !names.length}
                aria-invalid={!!invalid}
                aria-describedby={invalid && dirty ? errorId : undefined}
                onChange={(event) => {
              ...
              <button
                ...
                disabled={locked || !names.length || draft.winnerCount >= names.length}
                onClick={() => adjustCount(1)}
              >
                +
              </button>
          ...
          <div className={styles.actions}>
            ...
            >
              {dirty ? `저장하고 ${countLabel}명 추첨` : `${countLabel}명 추첨 시작`}
            </Button>
            {dirty && (
              <button
                type="button"
                className={styles.saveOnly}
                disabled={locked || !!invalid}
        ...
        <div className={styles.readOnlyCount}>
          <span>당첨 인원</span>
          <strong>{room.winnerCount}명</strong>
        </div>
      )}
    </section>
  )
