...
vi.mock('../../model/useLotteryChat', () => ({
  useLotteryChat: (props: unknown) => state.useChat(props),
}))
...
const view = () => (
  <LotteryChatPanel transport={transport} currentUser={members[0]} members={members} />
)
const rect = (top: number, height: number, width = 320) => ({
  top,
  bottom: top + height,
  left: 0,
  right: width,
...
describe('LotteryChatPanel', () => {
  ...
  it('uses the final input event after compositionend and prevents an Enter newline', async () => {
    ...
    const newline = new InputEvent('beforeinput', {
      bubbles: true,
      cancelable: true,
  ...
  it('reveals actual reader names and retries a failed message by its original client id', async () => {
    chat.messages = [
      makeMessage(1, { senderId: 1, senderName: '선도우', readers: [members[1]] }),
      makeMessage(-1, {
        senderId: 1,
        senderName: '선도우',
        delivery: 'failed',
        failure: '전송을 확인하지 못했어요.',
      }),
    ]
    const user = userEvent.setup()
    render(view())
  ...
  it('keeps the scroll position while reading older messages and offers an explicit latest jump', async () => {
    ...
    chat = { ...chat, messages: [...chat.messages, makeMessage(3)], unreadCount: 1 }
    rerender(view())
  ...
  it('preserves the visible anchor when an older server page is prepended', async () => {
    chat.messages = [makeMessage(101), makeMessage(102)]
    chat.hasMore = true
    let height = 1000
    const user = userEvent.setup(),
      { rerender } = render(view())
    ...
    chat = { ...chat, loadingOlder: true }
    rerender(view())
    height = 1200
    chat = { ...chat, loadingOlder: false, messages: [makeMessage(100), ...chat.messages] }
    rerender(view())
  ...
  it('returns to the full conversation and follows the sent message after a filtered send', async () => {
    ...
    chat = {
      ...
      senderId: null,
      query: '',
      messages: [...chat.messages, makeMessage(3, { senderId: 1 })],
    }
    rerender(view())
    expect(container.scrollTop).toBe(1000)
  })
