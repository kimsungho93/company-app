...
export type OrbitHandle = {
  ...
  getAnchor(userId: number): DOMRect | undefined
}
type Props = {
  members: ChatPerson[]
...
export const OrbitParticipants = ({
  members,
  currentUserId,
  selectedId,
  typing,
  ...
  const visible = useMemo(() => {
    ...
    if (chosen && !result.some((person) => person.userId === chosen.userId))
      result[Math.max(0, result.length - (own ? 2 : 1))] = chosen
    return result
  }, [members, currentUserId, selectedId])
  const visibleRef = useRef(visible)
  visibleRef.current = visible
  const overflow = members.filter(
    (person) => !visible.some((shown) => shown.userId === person.userId),
  )
  ...
  const fallbackLayout = useCallback(() => {
    ...
    layout(
      people.map((person, index) => {
        ...
        return {
          id: person.userId,
          x: width / 2 + Math.sin(angle) * width * 0.33,
          y: 88 + Math.cos(angle) * 53,
          scale: 1,
  ...
  const readColors = useCallback((): OrbitColors => {
    ...
    const colors = {
      ...
      ring: resolve('--border-input'),
      platform: resolve('--bg-sunken'),
    }
    probe.remove()
    return colors
  }, [])
  ...
  useEffect(() => {
    ...
    void import('./orbitScene')
      .then(({ createLotteryOrbitScene }) => {
        if (cancelled || !host.current) return
        const instance = createLotteryOrbitScene({
          host: host.current,
          participants: visibleRef.current.map((person) => ({
            id: person.userId,
            name: person.name,
            color: chatColor(person.userId),
          })),
          colors: readColors(),
          onLayout: layout,
  ...
  const typingId =
    typing.find((person) => visible.some((member) => member.userId === person.userId))?.userId ??
    null
  useEffect(() => {
    scene.current?.setTyping(typingId)
  }, [typingId, ready])
  useEffect(() => {
    if (scene.current) scene.current.updateColors(readColors())
  }, [theme, readColors])
  useImperativeHandle(
    ref,
    () => ({
      pulse: (id) => scene.current?.pulse(id),
      getAnchor: (id) => buttons.current.get(id)?.getBoundingClientRect(),
    }),
    [],
  )
  ...
  return (
    <div className={styles.orbit}>
      <div
        ...
        data-ready={ready}
        data-testid="chat-orbit"
        aria-label="함께 있는 사람의 궤도"
        ...
        <div className={styles.projection}>
          {visible.map((person) => (
            <button
              key={person.userId}
              ref={(element) => {
                if (element) buttons.current.set(person.userId, element)
                else buttons.current.delete(person.userId)
              }}
              type="button"
              data-orbit-person={person.userId}
              data-typing={typing.some((value) => value.userId === person.userId)}
              className={styles.person}
              aria-label={`${person.name}${person.userId === currentUserId ? ', 나' : ''} 대화 보기`}
              aria-pressed={selectedId === person.userId}
              style={{ '--person-color': chatColor(person.userId) } as CSSProperties}
              onClick={() => {
                if (performance.now() >= suppressClickUntil.current)
                  onSelect(selectedId === person.userId ? null : person.userId)
              }}
            >
              <span className={styles.sphereHit} aria-hidden="true">
                <span className={styles.fallbackBall} />
              </span>
              <span className={styles.personName}>
                {person.userId === currentUserId ? '나' : person.name}
              </span>
            </button>
          ))}
        </div>
      ...
      <div className={styles.rotate} aria-label="참여자 궤도 회전">
        ...
        {overflow.length > 0 && (
          <details className={styles.overflow}>
            <summary>+{overflow.length}명</summary>
            <div>
              {overflow.map((person) => (
                <button type="button" key={person.userId} onClick={() => onSelect(person.userId)}>
                  {person.name} 대화 보기
                </button>
              ))}
            </div>
