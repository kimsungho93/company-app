...
const Message = ({
  ...
  return (
    <article
      className={`${styles.message} ${own ? styles.own : ''}`}
      data-chat-seq={message.seq}
      data-readable={message.delivery === 'sent'}
      aria-label={`${message.senderName} ${timeLabel(message.sentAt)}`}
    >
      {!own && (
        <span
          className={styles.dot}
          aria-hidden="true"
          style={{ '--person-color': chatColor(message.senderId) } as CSSProperties}
        />
      )}
              ...
              >
                <span className={styles.beads} aria-hidden="true">
                  {message.readers.slice(0, 5).map((reader) => (
                    <span
                      key={reader.userId}
                      style={{ '--person-color': chatColor(reader.userId) } as CSSProperties}
                    />
                  ))}
                </span>
                {message.readers.length}명 읽음
              </button>
            ) : (
              <span>전송됨</span>
            ))}
        </div>
        {own && message.delivery === 'failed' && message.failure && (
          <p className={styles.messageFailure}>{message.failure}</p>
        )}
        {own && message.readers.length > 0 && (
          <div id={readersId} className={styles.readerDetail} hidden={!readersOpen}>
...
export const LotteryChatPanel = ({ transport, currentUser, members }: Props) => {
  ...
  const selectedName =
    selectedPerson?.name ??
    chat.messages.find((message) => message.senderId === chat.senderId)?.senderName ??
    '선택한 사람'
  ...
  const requestRead = useCallback(() => {
    ...
    readFrame.current = requestAnimationFrame(() => {
      ...
      const seqs = [...container.querySelectorAll<HTMLElement>('[data-readable="true"]')]
        .filter((element) => {
          ...
          return (
            ...
            Math.min(rect.bottom, bottom) - Math.max(rect.top, top) >=
              Math.min(rect.height, bottom - top) * 0.6 &&
            Math.min(rect.right, right) > Math.max(rect.left, left)
          )
        })
        .map((element) => Number(element.dataset.chatSeq))
        .filter((seq) => seq > 0)
        .slice(0, 100)
  ...
  useEffect(() => {
    ...
    const observer =
      typeof IntersectionObserver === 'undefined'
        ? null
        : new IntersectionObserver(requestRead, { threshold: [0, 0.6, 1] })
    list.current
      .querySelectorAll('[data-readable="true"]')
    ...
    return () => {
      ...
      window.removeEventListener('scroll', requestRead, true)
  ...
  useLayoutEffect(() => {
    ...
    if (saved && !chat.loadingOlder) {
      if (chat.messages[0]?.seq !== saved.oldest)
        container.scrollTop = saved.top + container.scrollHeight - saved.height
      anchor.current = null
  ...
  useEffect(
    () => () => {
      for (const [animation, element] of flights.current) {
        animation.cancel()
        element.remove()
      }
  ...
  const flyToOrbit = () => {
    const host = panel.current,
      start = sendButton.current?.getBoundingClientRect(),
      finish = orbit.current?.getAnchor(currentUser.userId)
    if (!host || !start || !finish || reduced || typeof Element.prototype.animate !== 'function') {
      orbit.current?.pulse(currentUser.userId)
      return
    }
    const bounds = host.getBoundingClientRect(),
      ball = document.createElement('span')
    ball.className = styles.flight
    ball.setAttribute('aria-hidden', 'true')
    host.appendChild(ball)
    const sx = start.left - bounds.left + start.width / 2 - 7,
      sy = start.top - bounds.top + start.height / 2 - 7
    const fx = finish.left - bounds.left + finish.width / 2 - 7,
      fy = finish.top - bounds.top + 16
    const animation = ball.animate(
      [
        { transform: `translate(${sx}px, ${sy}px) scale(.7)`, opacity: 0.8 },
        {
          transform: `translate(${sx + 25}px, ${(sy + fy) / 2}px) scale(1.1)`,
          opacity: 1,
          offset: 0.45,
        },
        { transform: `translate(${fx}px, ${fy}px) scale(.3)`, opacity: 0.4 },
      ],
      { duration: 580, easing: 'cubic-bezier(.3,.65,.3,1)' },
    )
    ...
    void animation.finished
      ...
      .catch(() => undefined)
      .finally(() => {
        flights.current.delete(animation)
        ball.remove()
      })
  }
  ...
  const send = () => {
    ...
    if (composing.current || !chat.connected || !value.trim() || codepoints(value).length > 300)
      return
  ...
  const toggleSearch = () => {
    if (searchOpen) {
      ...
      setSearchOpen(true)
      requestAnimationFrame(() => searchField.current?.focus({ preventScroll: true }))
    }
  }
  const mention = () => {
  ...
  const jumpToLatest = () => {
    following.current = true
    anchor.current = null
    if (chat.query || chat.senderId !== null) {
      setSearchInput('')
      chat.search('')
      chat.filterSender(null)
    }
  ...
  return (
    <section
      ...
      data-lottery-chat="true"
    >
      <header className={styles.header}>
        <h2 id={titleId}>채팅</h2>
        <span className={styles.online}>{members.length}명 함께</span>
        ...
        {open && (
          <button
            ...
            aria-label={searchOpen ? '메시지 검색 닫기' : '메시지 검색 열기'}
          ...
          <>
            {!searchOpen && (
              <OrbitParticipants
                ...
                selectedId={chat.senderId}
                typing={chat.typing}
                onSelect={chat.filterSender}
              />
            )}
            <div className={styles.viewbar}>
              <span>{chat.senderId === null ? '전체 대화' : `${selectedName}의 대화`}</span>
              <div>
                {selectedPerson && selectedPerson.userId !== currentUser.userId && (
                  <button type="button" className={styles.textButton} onClick={mention}>
                    언급하기
                  </button>
                )}
                {chat.senderId !== null && (
                  <button
                    ...
                    onClick={() => chat.filterSender(null)}
                  >
                    모두 보기
                  </button>
                )}
            ...
            {searchOpen && (
              <div id={searchId} className={styles.search}>
                <input
                  ref={searchField}
                  type="search"
                  aria-label="메시지 검색"
                  ...
                  onChange={(event) => {
                    ...
                    chat.search(event.target.value)
                  }}
                  onKeyDown={(event) => {
            ...
            {chat.error && (
              <div className={styles.error}>
                ...
                <button
                  ...
                  onClick={() => chat.search(chat.query)}
                >
                  다시 불러오기
                </button>
              </div>
            ...
            <div
              ...
              aria-label="대화 목록"
              aria-busy={chat.loading || chat.loadingOlder}
              onScroll={() => {
            ...
            >
              ...
              {!chat.loading && chat.connected && !chat.error && chat.messages.length === 0 && (
                <p className={styles.empty}>
                  {chat.query || chat.senderId !== null
                    ? '일치하는 메시지가 없어요.'
                    : '첫 메시지를 남겨 보세요.'}
                </p>
              )}
              {chat.messages.map((message) => (
                <Message
                  key={`${message.senderId}:${message.clientMessageId || message.id}`}
                  message={message}
                  own={message.senderId === currentUser.userId}
                  query={chat.query}
            ...
            {!atBottom && (
              <button type="button" className={styles.jump} onClick={jumpToLatest}>
                {chat.unreadCount ? `새 메시지 ${chat.unreadCount}개` : '최신 대화로'} ↓
              </button>
            )}
            <div className={styles.typing} role="status">
              {chat.typing.length > 0 &&
                `${chat.typing
                  ...
                  .join(
                    ', ',
                  )}${chat.typing.length > 2 ? ` 외 ${chat.typing.length - 2}명` : ''} 입력 중…`}
            </div>
            {draftError && (
            ...
            >
              <div>
                ...
                <textarea
                  ...
                  onKeyDown={onKeyDown}
                  onChange={(event) => {
                    const value = composing.current
                      ? event.target.value
                      : codepoints(event.target.value).slice(0, 300).join('')
                    setDraft(value)
                    setDraftError(null)
                  ...
                  onCompositionEnd={(event) => {
                    ...
                    setDraft(codepoints(event.currentTarget.value).slice(0, 300).join(''))
                    if (pendingSend.current) scheduleSend()
                  }}
                  onBlur={() => {
                    cancelPendingSend()
                    composing.current = false
                ...
                />
                {count >= 240 && (
                  <span className={styles.count} aria-label={`${count}자, 최대 300자`}>
                    {count} / 300
                  </span>
                )}
              </div>
              <button
                ...
                aria-label="메시지 보내기"
                disabled={!chat.connected || !draft.trim() || count > 300}
              >
                ↑
              </button>
            </form>
