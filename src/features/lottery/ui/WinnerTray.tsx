  ...
  <section className={`${styles.panel} ${styles.results}`} aria-labelledby="lottery-winners-title">
    <div className={styles.sectionHeading}>
      <div className={styles.resultHeading}>
        <h2 id="lottery-winners-title">{finished ? '오늘의 당첨자' : '당첨 결과'}</h2>
        <span className={styles.muted}>
          {winners.length} / {count}명
        </span>
      </div>
      {winners.length > 0 && (onCopy || extraActions) && (
    ...
    {winners.length ? (
      <ol className={styles.winners} aria-label="발표 순서대로 당첨자">
        {winners.map((winner, index) => (
          <li key={`${winner.name}-${winner.drawnAt}`} className={styles.winner}>
            <span className={styles.winnerOrder}>{index + 1}번째</span>
            <div
              className={styles.winnerBall}
              style={{ background: ballColor(Math.max(0, participants.indexOf(winner.name))) }}
            >
      ...
      <div className={styles.emptySlots} aria-hidden="true">
        {Array.from({ length: Math.min(count, 6) }, (_, index) => (
          <div key={index}>
            <span>{index + 1}</span>
          </div>
        ))}
        {count > 6 && <span className={styles.moreSlots}>+{count - 6}</span>}
      </div>
    )}
    <p className={styles.srOnly} role="status" aria-atomic="true">
      {winners.length
        ? `${winners.length}번째 당첨자 ${winners[winners.length - 1].name}${finished ? '. 추첨이 완료되었습니다.' : ''}`
        : '추첨을 기다리고 있습니다.'}
    </p>
