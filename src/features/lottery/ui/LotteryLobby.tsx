...
export const LotteryLobby = () => {
  ...
  const visibleRooms = rooms
    ?.filter(
      (room) =>
        filter === 'all' ||
        (filter === 'finished' ? room.status === 'FINISHED' : room.status !== 'FINISHED'),
    )
  ...
  return (
    <div className={styles.page}>
      <header className={styles.heading}>
        <div>
          <h1>사람 뽑기</h1>
          <p>추첨방을 만들거나 동료의 추첨에 참여하세요.</p>
        </div>
        <div className={styles.sampleBalls} aria-hidden="true">
          {[0, 1, 2].map((index) => (
            <span key={index} style={{ background: ballColor(index) }} />
          ))}
        </div>
      </header>
      <section
        className={`${styles.panel} ${styles.createPanel}`}
        aria-labelledby="lottery-create-title"
      >
      ...
      <section aria-labelledby="lottery-rooms-title">
        <div className={styles.sectionHeading}>
          <h2 id="lottery-rooms-title">추첨방 목록</h2>
          <Button
            variant="secondary"
            loading={refreshing}
            onClick={() => void refresh()}
            aria-label="추첨방 목록 새로고침"
          >
            새로고침
          </Button>
        </div>
        {(connectionStatus === 'reconnecting' || connectionStatus === 'authError') && (
          <p className={styles.muted} role="status">
            {connectionStatus === 'authError'
              ? '실시간 연결을 확인하지 못했습니다. 새로고침을 눌러 다시 연결해 주세요.'
              : '실시간 업데이트에 다시 연결 중이에요. 새로고침으로 목록을 확인할 수 있어요.'}
          </p>
        )}
        <div className={styles.filters} role="group" aria-label="추첨방 필터">
          {FILTERS.map((item) => (
            <button
              key={item.value}
              type="button"
        ...
        {isLoading ? (
          ...
          <div className={styles.empty}>
            <p role="alert">추첨방을 불러오지 못했습니다.</p>
          ...
          <div className={styles.roomGrid}>
            {visibleRooms.map((room) => (
              <Link key={room.id} to={`/games/lottery/${room.id}`} className={styles.roomCard}>
                <span
                  className={`${styles.badge} ${room.status === 'DRAWING' ? styles.liveBadge : room.status === 'FINISHED' ? styles.finishedBadge : ''}`}
                >
                  {STATUS_LABELS[room.status]}
                </span>
                <h3>{room.title}</h3>
                <p>{room.hostName}님의 추첨</p>
                <div className={styles.roomMeta}>
                  <span>
                    추첨 대상 {room.participantCount}명 · 당첨 {room.winnerCount}명
                  </span>
                  <strong>{room.status === 'FINISHED' ? '결과 보기' : '참여하기'} →</strong>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className={styles.empty}>
            ...
            {filter !== 'finished' && (
              <span className={styles.muted}>위에서 새 추첨방을 만들 수 있어요.</span>
            )}
          </div>
        )}
      </section>
