...
export const lotteryApi = baseApi
  .enhanceEndpoints({ addTagTypes: ['LotteryRooms'] })
  .injectEndpoints({
    endpoints: (build) => ({
      ...
      saveLotterySettings: build.mutation<
        LotteryRoomSnapshot,
        { id: string; settings: LotterySettings }
      >({
        query: ({ id, settings }) => ({
          url: `${roomPath(id)}/settings`,
          method: 'PUT',
          body: settings,
        }),
        invalidatesTags: ['LotteryRooms'],
