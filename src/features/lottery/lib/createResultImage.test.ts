...
type RenderedText = { text: string; x: number; y: number; width: number; size: number; align: CanvasTextAlign; color: string | CanvasGradient | CanvasPattern }

const input: LotteryResultImageInput = {
  title: '오늘의 추첨',
  participants: ['선도우', '육이슬', '김성호'],
  winners: [{ name: '김성호', drawnAt: '2026-09-14T00:00:00Z' }, { name: '선도우', drawnAt: '2026-09-14T00:00:04Z' }],
  winnerCount: 2,
  finished: true,
}
...
describe('createResultImage', () => {
  ...
  beforeEach(() => {
    ...
    const textWidth = (text: string) => Array.from(text).reduce((width, character) => width + ((character.codePointAt(0) ?? 0) > 127 ? 1 : 0.6) * fontSize(), 0)
    context = {
      font: '',
      fillStyle: '',
      textAlign: 'left',
      textBaseline: 'top',
      beginPath: vi.fn(() => { isBall = false }),
      roundRect: vi.fn(),
      ellipse: vi.fn(),
      arc: vi.fn(() => { isBall = true }),
      fill: vi.fn(() => { if (isBall && typeof context.fillStyle === 'string') colors.push(context.fillStyle) }),
      fillRect: vi.fn(),
      createRadialGradient: vi.fn(() => ({ addColorStop: vi.fn() })),
      measureText: vi.fn((text: string) => ({ width: textWidth(text) })),
      fillText: vi.fn((text: string, x: number, y: number) => {
        rendered.push({ text, x, y, width: textWidth(text), size: fontSize(), align: context.textAlign, color: context.fillStyle })
      }),
  ...
  it('exports a PNG with the presented order and each participant’s existing color', async () => {
    ...
    expect(rendered.filter(({ size, color }) => size === 34 && color === '#191f28').map(({ text }) => text)).toEqual(['김성호', '선도우'])
  ...
  it('keeps all fifty long names and a sixty-character title without truncation or canvas overflow', async () => {
    ...
    const participants = Array.from({ length: 50 }, (_, index) => `${String(index + 1).padStart(2, '0')}${'김'.repeat(28)}`)
    const winners = [...participants].reverse().map((name) => ({ name, drawnAt: '2026-09-14T00:00:00Z' }))
    await createResultImage({ title, participants, winners, winnerCount: 50, finished: true })
    const canvas = vi.mocked(HTMLCanvasElement.prototype.getContext).mock.contexts[0] as HTMLCanvasElement

    expect(rendered.filter(({ size }) => size === 46).map(({ text }) => text).join('')).toBe(title)
    expect(rendered.filter(({ size }) => size === 34).map(({ text }) => text).join('')).toBe(winners.map(({ name }) => name).join(''))
    expect(rendered.filter(({ text }) => /^\d+번째$/.test(text)).map(({ text }) => text)).toEqual(Array.from({ length: 50 }, (_, index) => `${index + 1}번째`))
  ...
  it('waits for the Korean font before measuring or drawing the export', async () => {
    ...
    fonts.load.mockReturnValue(new Promise<void>((resolve) => { releaseFont = resolve }))
    const exportImage = createResultImage(input)
    expect(HTMLCanvasElement.prototype.getContext).not.toHaveBeenCalled()
    expect(fonts.load).toHaveBeenCalledWith(expect.stringContaining('Wanted Sans Variable'), expect.stringContaining('김성호'))
    releaseFont?.()
    await expect(exportImage).resolves.toBe(png)
  })

  it('rejects an empty result before creating an image', async () => {
    await expect(createResultImage({ ...input, winners: [] })).rejects.toThrow('저장할 당첨 결과가 없습니다.')
    expect(HTMLCanvasElement.prototype.getContext).not.toHaveBeenCalled()
  })
  ...
  it('rejects a thrown encoder error so the caller can offer a retry', async () => {
    vi.mocked(HTMLCanvasElement.prototype.toBlob).mockImplementation(() => { throw new Error('Encoding failed') })
