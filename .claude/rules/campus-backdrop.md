...
- 정적 환경은 `campusEnvironment`, 수목·재질은 `campusLandscape`·`campusSurfaceTextures`, 사람·차량 조립과 이동은 `campusActorModels`·`campusActors`, 경로·게이트 계산은 `campusMotion`, 렌더링 수명주기는 `campusRenderer`가 담당한다.
...
- 해제 시 RAF·관찰자·이벤트·Three.js geometry/material/texture·InstancedMesh·그림자·WebGL 동기화 객체와 렌더러를 정리한다. 공유 자원은 중복 해제하지 않는다. 하늘 환경맵의 렌더 타깃과 CI 이미지 로딩 콜백도 해제한다.
