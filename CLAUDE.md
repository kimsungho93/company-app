...
### 방에 들어갈 때는 반드시 `POST /join` 을 부른다

**이동만 하면 안 된다.** 서버가 SUBSCRIBE 프레임에서 참가자인지 검사하므로, 참가자가 아닌 채로
소켓만 붙으면 구독이 거절되고 화면은 "연결이 끊어졌다"로 읽어 목록으로 튕긴다.

잠긴 방은 `JoinRoomDialog` 가 부르고, **잠기지 않은 방은 `WordChainRoomsPage` 가 부른다.**
...
0.5초는 17%다. 그 값을 CSS 애니메이션 길이로 주고 `key={turnEndsAt}` 로 턴이 바뀔 때만
리마운트한다 — `requestAnimationFrame` 으로 세면 3초마다 180번 리렌더가 난다.

**한글 조합 중 Enter 는 조합만 확정하고 제출하지 않는다.** 한 번 더 쳐야 하는데 3초에서는 그
사이에 턴이 끝나 기회를 통째로 잃는다. `compositionend` 에서 흘려보내되, 그 깃발을
`compositionstart` 와 차례가 바뀔 때 **반드시 내려야** 다음 턴으로 새어 나가지 않는다.
`compositionend` 에서는 state 가 아니라 `currentTarget.value` 를 읽는다 — 순서가 브라우저마다
달라 state 는 한 글자 뒤처질 수 있다.

**옵션(`turnSeconds`·`noReuse`)은 `game` 이 아니라 방에 있다.** 한 판도 안 한 방은 `game` 이
`null` 이라 실을 자리가 없고, 화면이 완전 제어 컴포넌트라 방장이 고른 값이 **본인 라디오에서도
되돌아간다.**

**내 답이 `PENDING` 인 동안 입력칸을 잠근다.** 안 잠그면 서버가 둘 다 세어 기회가 한 번에 둘
날아가고, 늦게 온 판정이 대기 상태를 지워 시간 안에 낸 사람이 시간 초과로 잡힌다.

**판이 끝나면 무대까지 대기실로 돌린다** — `Stage` 에 `game` 을 넘기지 않는다. 게임 모드 연단은
준비 배지를 감추므로, 안 그러면 준비를 눌러도 화면에 아무 변화가 없고 `탈락` 배지가 남아 다음
판 방장이 회색으로 죽어 보인다. 승자는 배너가 이름으로 알린다.

**말풍선은 흐름 밖에 있다.** 안에 두면 말풍선 너비가 좌석 너비가 되어 누가 말할 때마다 연단이
가로로 밀린다. `left: 50%` 는 가용 너비를 절반으로 자르므로 `width: max-content` 가 같이 필요하다.

### 아바타

7개다. `public/avatars/*.png` 이고 경로·라벨은 `model/avatars.ts` 한 곳에 있다.
연단과 선택 UI 가 같은 표를 본다.

**`Avatar` 는 백엔드 `enum Avatar` 와 한 벌이다.** 프론트에만 값을 늘리면 그 아바타는
**눌러도 아무 일이 안 일어난다** — 서버가 역직렬화에서 조용히 버린다(실측). ERROR 프레임도
안 오고 소켓도 안 끊기므로 화면에 아무 흔적이 안 남는다. 백엔드가 먼저다.

**선택 UI 는 줄바꿈한다.** 7개면 364px 라 `ReadyBar` 가 좁아지면 한 줄에 안 들어간다.

**안 고른 쪽을 흐리게 물리는 것은 장식이 아니다.** 이미지 안쪽에 링을 그렸더니 아바타 배경
채도에 묻혀 어느 쪽을 골랐는지 보이지 않았다. jsdom 에는 픽셀이 없어 테스트가 못 잡는다.

## 브라우저로 확인할 때 나를 속이는 것들

화면을 눈으로 못 보고 DOM 과 계산된 스타일만 읽을 때, 아래 둘에 실제로 걸렸다.

**패널이 안 보이면 `document.visibilityState` 가 `hidden` 이고, 그때 브라우저는 스타일
재계산을 미룬다.** `getComputedStyle` 이 낡은 값을 그대로 돌려준다. 이걸 모르고 멀쩡한
CSS 를 버그로 오진해 구현을 한 번 갈아엎었다가 되돌렸다.

**셀렉터 매칭(`el.matches(':checked')`)은 정상이라 둘이 어긋나 보이는 게 신호다.**
읽기 전에 강제로 리플로를 끼우면 맞는 값이 나온다.

```js
el.style.display = 'none'; void el.offsetHeight; el.style.display = ''
```

**같은 자리에 같은 모양으로 다시 그려지는 것을 기다리면 직전 결과와 구분이 안 된다.**
말풍선으로 사전 캐시 적중을 재려다 `1ms` 를 얻었는데, 2차 제출 직후에도 화면에는 1차
말풍선이 그대로 남아 있어 그걸 즉시 잡은 것이었다. 새 판정을 기다린 게 아니다.
같은 단어·같은 판정이면 글자로는 갈리지 않는다. **서버 로그처럼 화면 밖 증거를 쓴다.**
